...
object WsServer {
  ...
  /**
    ...
    *
    */
  def counterResponse[F[_]](countTo: Int, clientId: String)(implicit F: Effect[F], ec: ExecutionContext, S: Scheduler): F[Response[F]] = {
              ...
              counter.modify(_ + 1)
                .flatMap {
                  case async.Ref.Change(_, now) =>
                    messageQueue.enqueue1(Text(s"$clientId: $now"))
                      .flatMap { _ =>
                        if (now >= countTo) {
    ...
    def fromClient(
                  ...
                  ): Sink[F, WebSocketFrame] = _.evalMap {
      case Text(t, _) =>
  ...
  def service[F[_]](implicit F: Effect[F], ec: ExecutionContext, S: Scheduler): HttpService[F] = {
    ...
    HttpService[F] {
      case GET -> Root / "updates" / countTo =>
        for {
          int <- Sync[F].delay(countTo.length)
          resp <- counterResponse(int, countTo)
        } yield resp
    }
  }
